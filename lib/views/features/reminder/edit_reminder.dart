...
class EditReminderView extends StatefulWidget {
  ...
  const EditReminderView({super.key, required this.reminder});
...
class _EditReminderViewState extends State<EditReminderView> {
  ...
  late final CloudReminder _reminder;
  ...
  void initState() {
    ...
    by = DateTime.parse(_reminder.by);
    isCompleted = _reminder.isCompleted == 1;
    _titleController.text = _reminder.title;
    doRemind = _reminder.doRemind == 1;
    remindAt = DateTime.parse(_reminder.remindAt);
    doRepeat = _reminder.doRepeat == 1;
    _selectedRepeat = _reminder.repeat;
  ...
  Future _saveForm() async {
    ...
    if (_titleController.text.isNotEmpty) {
      ...
      final uniqueId = reminder.uniqueId;
      ...
      notifyHelper.cancelScheduledNotification(reminder.uniqueId);
      if (this.doRemind) {
        if (doRepeat) {
          if (_selectedRepeat == "Daily") {
            notifyHelper.scheduledDailyReminderNotification(this.remindAt, uniqueId, title);
          } else if (_selectedRepeat == 'Weekly') {
            notifyHelper.scheduledWeeklyReminderNotification(this.remindAt, uniqueId, title);
          } else {
            notifyHelper.scheduledMonthlyReminderNotification(this.remindAt, uniqueId, title);
          }
        } else {
          notifyHelper.scheduledReminderNotification(this.remindAt, uniqueId, title);
        }
      }
